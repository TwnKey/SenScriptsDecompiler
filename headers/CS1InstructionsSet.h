...
class CS1Builder : public Builder
{
    ...
    class OPCode61: public Instruction{
        ...
        int idx_read = location+1;
        this->AddOperande(operande(addr,"short", ReadSubByteArray(content, addr,2)));
        this->AddOperande(operande(addr,"int", ReadSubByteArray(content, addr,4)));
        fP.sizeInBytes = idx_read-location;
        return fP;
    ...
    class OPCode64: public Instruction{
        functionParameters fP;
        int idx_read = location+1;
        unsigned char code = reader->content[idx_read];
        this->AddOperande(operande(addr,"byte", ReadSubByteArray(content, addr, 1)));;
        this->AddOperande(operande(addr,"short", ReadSubByteArray(content, addr,2)));
        this->AddOperande(operande(addr,"int", ReadSubByteArray(content, addr,4)));
    ...
    class OPCode67: public Instruction{
        functionParameters fP;
        int idx_read = location+1;
        uint code2bytes = readUInt16(idx_read, reader->content);
        this->AddOperande(operande(addr,"short", ReadSubByteArray(content, addr,2)));
        unsigned char code = reader->content[idx_read];
        this->AddOperande(operande(addr,"byte", ReadSubByteArray(content, addr, 1)));;
        switch (code)  {
        ...
        if ((code2bytes != 0xFFFF) &&(code2bytes <= 0x96)&& (code == 0x6)){
            this->AddOperande(operande(addr,"short", ReadSubByteArray(content, addr,2)));
        }
        fP.sizeInBytes = idx_read-location;
        return fP;
    ...
    //0x6A
    functionParameters Menu: public Instruction{
        functionParameters fP;
        int idx_read = location+1;

        unsigned char code = reader->content[idx_read];
        this->AddOperande(operande(addr,"byte", ReadSubByteArray(content, addr, 1)));;
        unsigned char code2 = reader->content[idx_read];
        this->AddOperande(operande(addr,"byte", ReadSubByteArray(content, addr, 1)));;
        if (code2 < 4){
            switch (code)  {
                case 0x00:{
                ...
                case 0x01:{ //Texte des choix possibles
                    QByteArray text = readArray(idx_read,reader->content);
                    DialogLine choice = DialogLine(QString(text), idx_read);
                    choice.setType(LineType::Menu);
                    reader->currentFile.AddLine(choice);
                    this->AddOperande(operande(addr,"string", ReadStringSubByteArray(content, addr)));
