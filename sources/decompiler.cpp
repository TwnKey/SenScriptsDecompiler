...
bool Decompiler::check_all_files(const QString& log_filename,
    ...
    for (const auto& file_ : files) {
        ...
        std::vector<int> idx_fun_1, idx_fun_2;
        for (int i = idx_addresses_loc1; i < idx_addresses_loc1+size_pointer_section; i+=4) idx_fun_1.push_back(ReadIntegerFromByteArray(i, content1));
        for (int i = idx_addresses_loc2; i < idx_addresses_loc2+size_pointer_section; i+=4) idx_fun_2.push_back(ReadIntegerFromByteArray(i, content2));
        ...
        for (size_t i = 0; i < (uint) current_tf.getNbFunctions(); ++i){
            ...
            for (size_t j = 0; j < (uint) current_tf.FunctionsInFile[i].InstructionsInFunction.size(); ++j){
                ...
                for (size_t k = 0; k < (uint) current_tf.FunctionsInFile[i].InstructionsInFunction[j]->operandes.size(); ++k){
                    ...
                    {
                        for (int idx_b = 0; idx_b < bytes.size(); idx_b++){

                            uint8_t byte1 = content2[index_byte];
                            uint8_t byte2 = bytes[idx_b];
        ...
        stream << " Size 1: " << Qt::hex << content1.size() << " vs Size 2: " << Qt::hex << content2.size() << " " << success;
    }
return true;
}
